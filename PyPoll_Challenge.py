...
with open(file_to_save, "w") as txt_file:
    ...
    for county_name in county_votes:
        ...
        temp_votes = county_votes[county_name]
        # 6c: Calculate the percentage of votes for the county.
        county_vote_percentage = float(temp_votes) / float(total_votes) * 100



         # 6d: Print the county results to the terminal.
        county_results = (
                f"{county_name}: {county_vote_percentage:.1f}% ({temp_votes:,})")
        print(county_results)
            # 6e: Save the county votes to a text file.
        txt_file.write(str(temp_votes))
            # 6f: Write an if statement to determine the winning county and get its vote count.
        if (temp_votes > largest_county_vote):
                winning_count = temp_votes
                winning_percentage = county_vote_percentage
                largest_county = county_name
                largest_county_vote = temp_votes
    ...
    print(largest_county_print, end="")
